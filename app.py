...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col_mapa:
        ...
        custom_cmap = mcolors.LinearSegmentedColormap.from_list("custom", cmap_colors, N=256)
        ...
        fig_mapa.add_trace(go.Scattergeo(
            ...
            marker=dict(
                color=[vmin, vmax],
                colorscale=[[i/(len(colorscale)-1) if len(colorscale) > 1 else 0, c] for i, c in enumerate(colorscale)],
                showscale=True,
                cmin=vmin, cmax=vmax,
                colorbar=dict(
