...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col_mapa:
        ...
        def interpolar_color(t, palette):
            n = len(palette)
            if n == 1:
                return hex_a_rgb(palette[0])
            scaled = t * (n - 1)
            idx = min(int(scaled), n - 2)
            frac = scaled - idx
            r1, g1, b1 = hex_a_rgb(palette[idx])
            r2, g2, b2 = hex_a_rgb(palette[idx + 1])
            return (int(r1 + frac*(r2-r1)), int(g1 + frac*(g2-g1)), int(b1 + frac*(b2-b1)))

        def valor_a_color(v, vmin, vmax):
            t = 0.5 if vmax == vmin else (v - vmin) / (vmax - vmin)
            r, g, b = interpolar_color(t, PALETAS_MAPA[metrica])
